...
#        self._parent = parent
    ...
    def generate_waveform(self, filename):
        self.sample_generator = decoder.GstAudioFile(filename, format = 'S16LE', timeout = 2, samplerate = 250)
        self._duration = None
        self._force_stop = False
        self._data_points = []
        with self.sample_generator as generator:
...
#        return  True#self.srcpad.send_event(event)
    ...
    def _sink_chain(self, buf):
        ...
        time_per_packet = int(1.0 / self.stream_rate * 1000000000) # in nanoseconds
            ...
            #self._duration = 
            #self._duration = [1]
        self._packets_per_data_point = 1 #self._duration / (time_per_packet * self._num_data_points * 2)
