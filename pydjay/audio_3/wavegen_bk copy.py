...
#    __gsttemplates__ = (_sinktemplate, )
    ...
    def __init__(self, num_data_points, new_data_point_cb, process_done_cb, *args, **kwargs):   
...
#        self._parent = parent
    ...
    def generate_waveform(self, filename):
        self.sample_generator = decoder.GstAudioFile(filename)
...
#        return  True#self.srcpad.send_event(event)
    ...
    def _sink_chain(self, buf):
        ...
        if self._duration is not None:
            ...
            #if foo[0]:
            #self._duration = 
            self._duration = foo[1]
            self._packets_per_data_point = self._duration / (time_per_packet * self._num_data_points * 2)
        ...
        #try:
            #assert res
        offset   = 0
        buffer_bytes = array.array('i', data)
        num_samples  =  len(buffer_bytes)
        packets      = []
        timestamp = pts
            
        while offset + self.num_channels < num_samples:
            self._packet_queue.append((timestamp, tuple(buffer_bytes[offset:offset+self.num_channels])))
    ...
    def _process_packet_queue(self):
        ...
        if self._duration is not None:
            while len(self._packet_queue) >= self._packets_per_data_point:#
                ...
                self._idx += 1
                new_data_point = (self._duration, average_timestamp, (max_points+min_points) / 2)
                #print new_data_point
                if self._new_data_point_cb is not None:
                    self._new_data_point_cb(*new_data_point)
                self._data_points.append((average_timestamp, (max_points+min_points) / 2))
