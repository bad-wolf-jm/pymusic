...
from pydjay.uix.list_view import RecycleView, RecycleAdapter
from pydjay.uix import list_view
from pydjay.gui.list_items.default_track_list_item import DefaultTrackItemView
...
class TrackData_2(EventDispatcher):
...
#"""
#
#class DefaultTrackItemView(LongPressButtonBehaviour, RelativeLayout, SelectableView):
...
#            self.title  = unicode(self._item)
            ...
            #self.length = seconds_to_human_readable(int(self._item.info.length /1000))
...
class LargeTrackList(BoxLayout):
    ...
    def set_track_list(self, list, sort = True, available = None):
        ...
        _conv = self.item_convert if self.item_convert is not None else self._convert
        self.adapter.data = [_conv(i, x) for i, x in zip(range(len(bar)), bar)] #[self._convert(x) for x in bar]
