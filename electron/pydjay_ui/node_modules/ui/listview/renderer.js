...
class CellRenderer {
    ...
    _renderStyle(obj) {
        let styleElements = ["color", "backgroundColor", "textAlign", "fontSize", "maxWidth", "width"]
        let style = []
...
class TextRenderer extends CellRenderer {
    ...
    render(rowId, colId, obj) {
        ...
        let classList = this._getValue("class", obj)
        let id = this._config.id
        return `<td ${id ? `id='${id(rowId, colId)}'` : ""} class="${classList}" style="${style}">${this.formatNull(text)}</td>`
    }
