...
class AccordionHeader extends BoxLayout {
    constructor(label, actions) {
        super("horizontal", {left:0, right:0, top:0, botttom:0, padding:0})
        this.collapsed = true
        this.stub = new DOMElement("div")
        this.stub.setStyle({width:"7px"})
        this.collapsedIndicator = new Label()
        this.collapsedIndicator.setText(`<i class="fa fa-angle-right"></i>`)
        this.collapsedIndicator.domElement.classList.add("accordion-header-label")
        this.collapsedIndicator.setStyle({
            fontSize: '12pt',
            lineHeight:'30px',
        })
        this.nameLabel = new Label()
        this.nameLabel.setText(label)
        this.nameLabel.domElement.classList.add("accordion-header-label")
        this.append(this.stub, false, true, 7)
        this.append(this.collapsedIndicator, false, true, 15)
        ...
        this.domElement.classList.add("accordion-section-header")
    }
}
...
class AccordionLayout extends DOMElement {
    ...
    getPanelHeader(panel) {
        return this.headers[panel.domElement.id]
    }
    ...
    openPanel(panel) {
        if (this.panelOpen[panel.domElement.id]) {
            return
        }
        let height;
        ...
        this.headers[panel.domElement.id].collapsedIndicator.setText(`<i class="fa fa-angle-down"></i>`)
    }
    ...
    closePanel(panel) {
        ...
        let variableHeightPanels = this.getVariableHeightPanels()
        this._resizeVariableHeightPanels(variableHeightPanels, invariable_height, true)
        this.headers[panel.domElement.id].collapsedIndicator.setText(`<i class="fa fa-angle-right"></i>`)
    }
}
