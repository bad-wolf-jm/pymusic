...
class PymusicAppController extends EventDispatcher {
    ...
    setupMainPlayer() {
        ...
        this.main.on("track-started", async (log_data) => {
            await this.library.state.d.update({_id:"settings"}, {$set: {"current_track._id": log_data._id}})
        })
        ...
        this.main.on("track-skipped", async (log_data) => {
            await this.library.current_session.append(log_data.track_object, convertLogData(log_data, "SKIPPED"))
            await this.library.tracks.setTrackMetadata(log_data.track_object, updateTrackPlayedStats(log_data))
        })
    }

    setMasterOutputDeviceId(deviceId) {
        if (deviceId != this.getOutputDeviceIds()['master']) {
            this.main.setOutputDeviceId("master", deviceId)
        } 
    }

    setHeadsetOutputDeviceId(deviceId) {
        if (deviceId != this.getOutputDeviceIds()['headset']) {
    ...
    setupPrelistenPlayer() {
        this.prelisten = new AudioTrackPlayer()   
    ...
    playTrackAfterDelay(track, delay) {
        let id = setInterval(() => {
            if (delay <= 0) {
                ...
                this.dispatch("track-start-request", track)
    
            } else {
                this.dispatch('next-track-countdown', delay / 1000)
                delay = delay - 1000;
            }
        }, 1000)
    }
    ...
    async startQueue() {
        if (!this.queue_playing) {
            if (!(await this.library.queue.isEmpty())) {
                this.queue_playing = true
                this.dispatch("track-start-request", await this.library.queue.pop())
            }
        } 
    }
