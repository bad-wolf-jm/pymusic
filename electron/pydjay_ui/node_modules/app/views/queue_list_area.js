...
class QueueContextMenu extends ContextMenu {
    constructor(track, position, library, model) {
        ...
        this.menu.append(new MenuItem({label: 'Remove track', click: () =>{ 
            this.model.remove(this.position) 
        }}))
    }
}
...
class ListPane extends EventDispatcher {
    constructor(name, numTracks, duration, listArea, tableBody, library, headerElement) {
        super()
        this.nameDOM = name
        ...
        this.h = document.getElementById(headerElement)
        this.h.appendChild(this.infoHeader.domElement)
        ...
        if (this._dimUnavailableRows) {
            ...
            trackEntries.forEach((id) => {
                if (duplicateQueueEntries[id]) {
                    duplicateQueueEntries[id].push(positon)
                } else {
