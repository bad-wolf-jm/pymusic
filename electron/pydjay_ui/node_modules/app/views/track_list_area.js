...
class TrackListAreaController extends BoxLayout {
    constructor(element, library, queue) { 
        super('vertical')
        ...
        listScroller.domElement.classList.add("grid-table-scroller")
        // listScroller.domElement.classList.add("queue-list")

        let listTableArea = new DOMElement('table')
        listTableArea.domElement.classList.add("grid-table")
        listTableArea.domElement.classList.add("expand-width")
        // listTableArea.setStyle({width: "100%"})

        let listTableBody = new DOMElement('tbody')
        listTableBody.domElement.classList.add("expand-width")
        // listTableBody.setStyle({width: "100%"})

        listScroller.appendChild(listTableArea)
        listTableArea.appendChild(listTableBody)

        let listHeader =  new DOMElement('table')
        listHeader.domElement.classList.add("grid-table")
        listHeader.domElement.classList.add("expand-height")
        let listHeaderBody =  new DOMElement('thead')
        listHeaderBody.domElement.classList.add("expand-width")
        listHeader.appendChild(listHeaderBody)

        let listArea = new BoxLayout('vertical')
        listArea.append(listHeader, false, true, 25)
        listArea.append(new Separator(), false, true, 1)
        ...
        this.infoHeader.domElement.classList.add("grid-table-scroller")
        ...
        this._listview.on("filter-updated", (n) => {
            this.infoHeader.setResultCount(n)
        })
    ...
    async add_selection_to_queue() {
        ...
        if (selection.length == 1) {
            let T = await this._library.tracks.getElementById(selection[0])
            await this._queue.add_element(T)
        }
    }
