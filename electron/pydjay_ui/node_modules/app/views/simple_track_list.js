...
class SimpleTrackListView extends SortableListView {
    ...
    convert_track(x) {
        let element = (x == null) ? {id: null, duration: 0} : {
            id: x._id,
    ...
    dimmRow(pos, tag) {
        let e = this.getTableElementByPosition(pos)   
        let id = e.id 
        if (e != undefined) {
    ...
    dimmRowList(positionList, tag) {
        let idSet = {}
        positionList.forEach((t) => {idSet[t] = true})
    ...
    moveSelectedElement(amount) {
        if (this._selection.length() == 1) {
            ...
            let position = this._rendered_rows.getRowIndex(selectedId)
            if (position != -1) {
                let n = position + amount
                n = Math.max(n, 0)
                n = Math.min(n, this._rendered_rows.length() - 1) 
                let x = this._rendered_rows._ordering[position]
                this._rendered_rows._ordering[position] = this._rendered_rows._ordering[n]
                this._rendered_rows._ordering[n] = x
                this.sortable.sort(this._rendered_rows._ordering)
                this.listReordered(this._rendered_rows._ordering)
            }
        }
    }
    ...
    handleContextMenu(e) {
        let tableRow = e.target.closest("tr")
        let listIndex = this._displayed_rows.getRowIndex(tableRow.id)
        let trackObject = this._elements.getElementById(tableRow.id)
        this.dispatch("context-menu", trackObject.id, listIndex)
    }
}
