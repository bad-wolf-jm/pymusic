...
const { TrackMetadataDialog } = require("../dialogs/track_metadata_editor")
...
class ContextMenu {
    constructor(track, library) {
        ...
        this.menu = new Menu();
        (this.track.length == 1) && this.menu.append(new MenuItem({label: 'Get info', click: () => {
            AppController.stopPrelisten()
            let d = new TrackMetadataDialog(this.track[0], this.library)
            d.open()
        }}));
        ...
        (this.track.length == 1) && this.menu.append(new MenuItem({label: 'Clip track', click: async () => {
            ...
            let d = new TrackClipDialog(this.track[0], this.library)
