...
const { PropertySheet } = require("ui/dom/propertysheet")
...
class PrecuePlayerView extends StackLayout {
    constructor (track_list_model) {
        super()
        this.playerLayout = new BoxLayout('vertical', {left: 10, right: 10, top: 10, bottom: 5, padding: 15})
        ...
        this.trackPlayButton.domElement.classList.add('precue-management-button')
        this.trackProgress = new ProgressBar()
        this.trackProgress.setBounds(0, 100)
        this.trackProgress.domElement.classList.add('precue-player-progress')
        ...
        // let titleArtist = new BoxLayout("vertical", {padding:3})

        // controlLayout.append(seekLayout, true, false, {width: "100%", height: "25px"})

        let titleLayout =  new BoxLayout('vertical', {padding: 7})
        titleLayout.append(this.trackTitleLabel, false, true, 15)
        titleLayout.append(this.trackArtistLabel, false, true, 15)
        titleLayout.append(new DOMElement("div"), true, true)
        titleLayout.append(seekLayout, false, true, 25)
        controlLayout.append(titleLayout, true, true)
        ...
        infoLayout.addItem("<b>Played:</b>", this.trackLastPlayedLabel)
        infoLayout.addItem("<b>Count:</b>", this.trackPlayCountLabel)

        this.playerLayout.append(controlLayout, false, true, 76)
        this.playerLayout.append(ratingLayout, false, true, 15)
        this.playerLayout.append(infoLayout, true, true)
        this.addItem("player", this.playerLayout)
        ...
        this.overlay.domElement.innerHTML = "No track"
        this.addItem("overlay", this.overlay)
        this.displayItem("overlay")
    ...
    set_controller(controller) {
        ...
        this.controller.on("prelisten:stream-position", (pos) => {
            ...
            if (isFinite(percent)) {
                this.trackProgress.setValue(percent);
            }
        })
        ...
        this.controller.on("prelisten:playback-paused", () => {
            this.trackPlayButton.domElement.innerHTML = `<i class="fa fa-play"></i>`
        })
        this.controller.on("prelisten:playback-stopped", () => {
            this.trackPlayButton.domElement.innerHTML = `<i class="fa fa-play"></i>`
        })
        this.controller.on("prelisten:playback-started", () => {
            this.trackPlayButton.domElement.innerHTML = `<i class="fa fa-pause"></i>`
        })
        this.trackPlayButton.on('click', () => {
