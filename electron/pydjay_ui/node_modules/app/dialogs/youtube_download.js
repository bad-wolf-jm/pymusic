...
class YoutubeDownloadDialog extends Dialog {
    constructor(id, stream, track, library, config) {
        ...
        genreLayout.append(this.durationInfo, true, true)
        let downloadStack = new StackLayout()
        downloadStack.setStyle({
            background: 'rgb(175,175,175)'
        })
        let downloadBeginButton = new Button()
        downloadBeginButton.setText("Download")
        downloadBeginButton.on("click", async () => {
            let stream = ytdl(this.id, {
                quality: 'highestaudio',
                filter: 'audioonly',
            });
            let downloadTempFile = `${this.library.getTracksDownloadFolder()}/track_${this.id}.mp3`
            ...
            ffmpeg(stream)
                .audioBitrate(256)
                .save(downloadTempFile)
                .on('progress', (p) => {

                })
                .on('end', () => {
                    this.downloadedFile = downloadTempFile
                    save_button.enable()
                });
            downloadStack.displayItem("progress")
        })

        let downloadButtonLayout = new BoxLayout("horizontal", {top:15, bottom:15})
        downloadButtonLayout.append(new DOMElement("div"), true, true)
        downloadButtonLayout.append(downloadBeginButton, false, true, 200)
        downloadButtonLayout.append(new DOMElement("div"), true, true)
        downloadStack.addItem("start", downloadButtonLayout)

        let downloadProgressLayout = new BoxLayout("vertical", {left:15, right:15, top:15, bottom:15, padding:10})
        ...
        save_button.on("click", async () => {
            this.library.addFileWithMetadata(this.downloadedFile, this.getValues())
            this.close()
        })
        ...
        this.layout.append(downloadStack, false, true, 75)
