...
const { Separator } = require("ui/dom/separator")
...
class DJApplicationWindow extends ApplicationWindow {
    constructor() {
        ...
        this.queueListView = new QueueAreaController(this.queueListSpace.domElement, MDB)
        this.queueListView.domElement.addEventListener("click", () => {
            this.focusWindow(this.queueListView)
        })
        this.queueListView.hide_playlist_editor()
        this.queueListView.queue_view.displayModel("QUEUE", MDB.queue)
        this.queueListSpace.append(this.queueListView, true, true)



        this.mainListSpace = new BoxLayout('vertical')
        this.mainListView = new TrackListAreaController(this.mainListSpace.domElement, MDB, this.queueListView)
        this.mainListView._listview.on("row-click", () => {
            this.focusWindow(this.mainListView)
        })

        this.sidebarSpace = new Sidebar(this, MDB)
        this.mainListSpace.append(this.mainListView, true, true)
        this.queueListView.on("playlist-editor-closed", () => {
            this.mainListView.setDimmingModel(undefined)
        })



        let mainSpaceLayout = new BoxLayout("horizontal")
        mainSpaceLayout.setStyle({
            height: "calc(100vh - 110px)"
        })
        mainSpaceLayout.append(this.sidebarSpace, false, true, 350)
        mainSpaceLayout.append(new Separator(), false, true, 1)
        mainSpaceLayout.append(this.mainListSpace, true, true)
        mainSpaceLayout.append(new Separator(), false, true, 1)
        mainSpaceLayout.append(this.queueListSpace, false, true, 400)

        let footerSpace = new BoxLayout("horizontal")
        footerSpace.domElement.classList.add("footer")
        this.dateTimeLabel = new Label("")
        footerSpace.append(new DOMElement("div"), false, true, 10)
        footerSpace.append(this.dateTimeLabel, true, true)

        mainLayout.append(headerSpace, false, true, 85)
        mainLayout.append(mainSpaceLayout, true, true)
        mainLayout.append(footerSpace, false, true, 25)
               

        this.focusList = undefined
        this.lists = [this.mainListView, this.queueListView]
    ...
    initialize() {
        ...
        let startTime = () => {
            ...
            this.dateTimeLabel.setText(moment().format('MMMM Do YYYY | <b>h:mm:ss a</b>'))
