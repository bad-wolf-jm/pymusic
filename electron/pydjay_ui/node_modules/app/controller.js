...
class PymusicAppController extends EventDispatcher {
    ...
    setupMainPlayer() {
        ...
        let updateTrackPlayedStats = (log_data) => {return {
        ...
        this.main.on("track-started", async (log_data) => {
            ...
            this._current_track = log_data
            this.dispatch("main:track-started", log_data)
        })
    
        this.main.on("track-finished", async (log_data) => {
            await this.library.current_session.append(log_data.track_object, convertLogData(log_data, "FINISHED"))
            await this.library.history.append(log_data.track_object, convertLogData(log_data, "FINISHED"))
            await this.library.tracks.setTrackMetadata(log_data.track_object, updateTrackPlayedStats(log_data))
        ...
        this.main.on("track-stopped", async (log_data) => {
            await this.library.current_session.append(log_data.track_object, convertLogData(log_data, "STOPPED"))
            await this.library.history.append(log_data.track_object, convertLogData(log_data, "STOPPED"))
            await this.library.tracks.setTrackMetadata(log_data.track_object, updateTrackPlayedStats(log_data))
            this.dispatch("main:track-stopped", log_data)
        })
    
        this.main.on("stream-position", (track) => {
            this.dispatch("main:stream-position", track)
        })
    ...
    getPrelistenOutputDeviceId() {
        return this.prelisten.getOutputDeviceIds()["main"]
    }
    ...
    async startQueue() {
        if (!this.queue_playing) {
            console.log(await this.library.queue.isEmpty())
    ...
    startMainPlayback(track) {
        this.main.play(track, track.track.stream_start, track.track.stream_end)
    }
