...
class AudioOutputSettings extends Dialog {
    ...
    async open() {
        ...
        let audio_devices = await output_detector.detectAutioOutputs()
        audio_devices.sort((x,y) => {return x.label.localeCompare(y.label)})
        let audio_device_options = audio_devices.map((d) => {
            return {
                value: d.deviceId,
                label: d.label
            }
        })
        audio_device_options.splice(0, 0, {value:'null', label:"Disabled"})
        this.main_output_setting.setOptions(audio_device_options)
        this.main_output_setting.setValue(this.config.mainPlayer.getMasterOutputDeviceId())
        this.headset_output_setting.setOptions(audio_device_options)
        this.headset_output_setting.setValue(this.config.mainPlayer.getHeadsetOutputDeviceId())
        this.prelisten_output_setting.setOptions(audio_device_options)
        this.prelisten_output_setting.setValue(this.config.prelistenPlayer.getOutputDeviceId())
        console.log(this.config.prelistenPlayer.getOutputDeviceId())
