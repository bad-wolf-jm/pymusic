...
const {PydjayAudioContext} = require("./audio_context.js")
...
class PydjayAudioBasePlayer extends EventDispatcher {
    ...
    setOutputDeviceId(name, deviceId) {
        if (deviceId == 'null') {            
            ...
            this.audio_context.outputs[name].setSinkId("")
        } else {
    ...
    getOutputDeviceIds() {
        let outputs = {}
        for (name in this.audio_context.outputs) {
            outputs[name] = this.audio_context.outputs[name].sinkId
            if (outputs[name] == "") {
                outputs[name] = null
            }
        }
    ...
    muteOutput(name) {
        var a_monitor_volume = {volume: this.audio_context.outputs[name].volume};
        $(a_monitor_volume).animate(
            {volume: 0},
            {
                duration:250,
                step: (now, tween) => {
                    if (this.audio_context.outputs[name]) {
                        this.audio_context.outputs[name].volume = now
                    }
                },
            }
        )
...
module.exports = {
    PydjayAudioBasePlayer: PydjayAudioBasePlayer
}
