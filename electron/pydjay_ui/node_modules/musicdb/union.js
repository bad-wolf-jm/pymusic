...
class UnionModel extends TrackSetModel {
    constructor() {
        super()
        this.models = {}
        this.model_tracks = {}
    ...
    async addModel(name, model) {
        if (this.models[name] == undefined) {
            ...
            let t = await model.getTracks()
            this.model_tracks[name] = t.map((x) => {return (x != null) ? x._id : null})
            this.models[name].on("content-changed", async (q) => {
                let Q;
                if (q != undefined) {
                    Q = q.map((x) => {return (x != null) ? x._id : null})
                } else {
                    Q = []
                }
                ...
                tracks_to_remove.forEach((i) => {
                    if (current_tracks[i] == 1) {
                        this.dispatch("track-available", await this.models[name].getElementById(i))
                    }
                }) 
                tracks_to_add.forEach((i) => {
                    if (current_tracks[i] == undefined) {
                        this.dispatch("track-unavailable", await this.models[name].getElementById(i))
                    }
                }) 
