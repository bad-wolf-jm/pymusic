...
class TrackSetModel extends EventDispatcher {
    ...
    async isEmpty() {
        ...
        if ((element_list.ordering != undefined) && (element_list.ordering.indexOf(null) == 0)) {
            return true
        } else if ((element_list.ordering != undefined) && (element_list.ordering.length == 0)) {
            return true
        } else {
    ...
    async pop() {
        ...
        if ((element_list.ordering == undefined) || (element_list.ordering.length > 0) && (element_list.ordering[0] != null)) {
            ...
            let idx = element_list.ordering.indexOf(next_id)
            if (idx == -1) {
    ...
    async remove(track) {
        let element_list = await this._getElements()
        if (element_list.ordering != undefined) {
            let idx = Number.isInteger(track) ? track : element_list.ordering.indexOf(track._id)
            if (idx != -1) {
                element_list.ordering.splice(idx, 1)
            } 
            idx = element_list.ordering.indexOf(track._id)
            if (idx == -1) {
                delete element_list.elements[track._id]
            } 
        } else {
            delete element_list.elements[track._id]
        }
    ...
    async getOrderingIds() {
        let e = await this._getElements()
        let ids = e.ordering.filter((x) => {return true})
        return ids
    }
