...
const { CollectionController, TrackCollectionController, PlaylistCollectionController } = require("./collection.js")
const { QueueController }      = require("./queue.js")
const { SessionController }    = require("./session.js")
const { PlaylistController, QueryController }   = require("./playlist.js")
...
class MusicDatabase extends EventDispatcher {
    constructor(name) {
        super()
        this.tracks = new TrackCollectionController(path.join(".ds", name, "tracks.json"))
        this.sessions = new CollectionController(path.join(".ds", name, "sessions.json"))
        this.playlists = new PlaylistCollectionController(path.join(".ds", name, "playlists.json"))
        ...
        this.never_played_tracks = new QueryController(this, this.tracks, {$where: function () {
            return (this.history == undefined) || (this.history.length == 0)
        }})
        this.played_tracks = new QueryController(this, this.tracks, {$where: function () {
            return (this.history != undefined) && (this.history.length > 0)
        }})
    ...
    getTracksByIds(id_array) {
        ...
        return this.tracks.getIds(id_array)
