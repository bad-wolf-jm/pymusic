...
const { HistoryModel } = require("./history.js")
...
class MusicDatabase extends EventDispatcher {
    constructor(name, root) {
        ...
        this.history = new HistoryModel(this, this.tracks, path.join(this._path, "library", "history.json"))
        ...
        this.unavailableOrPlayed = new FunctionModel(this, this.tracks, async () => {
            let played_tracks = await this.current_session.getTrackIds()
            let unavailable_tracks = await this.unavailable_tracks.getTrackIds()
            let idx = {}
            played_tracks.forEach((x) => {idx[x] = true})
    ...
    getTracksDownloadFolder() {
        return path.join(this._path, "downloads")
    }
    ...
    async getTrackGap() {
        let settings = await this.getSettings()
        return settings.track_gap || 1000
    }

    async setTrackGap(ms) {
        await this.state.d.update({_id: "settings"}, {
            $set: {"track_gap": ms}
        })
    }
    ...
    getMetadataUpdates(metadata) {
        return {
            ...
            'track.stream_end': metadata.duration,
            'track.bitrate': metadata.bit_rate,
            'track.samplerate': metadata.samplerate,
            'track.bpm': metadata.bpm,
            'track.size': metadata.file_size,
        }
    }


    newTrack() {
        return this.tracks.d.insert({
            metadata: {
                title: null,
                artist: null,
                album: null,
    ...
    async addFileWithMetadata(file_path, metadataUpdate) {
        let newTrack = await this.newTrack()
        let fileMetadata = await read_metadata(file_path)
        fileMetadata = Object.assign(this.getMetadataUpdates(fileMetadata), metadataUpdate)
        let trackId = newTrack._id
        let track_filename = `${newTrack._id}.${this.getFileExtension(file_path)}`
        await fs.copy(file_path, path.join(this.getTracksFolder(), track_filename))
        fileMetadata["track.filename"] = track_filename
        await this.tracks.d.update({_id: trackId}, {$set: fileMetadata})
        this.tracks.dispatch("content-changed", await this.tracks.getTracks())
        return this.getTrackById(trackId)
    }
