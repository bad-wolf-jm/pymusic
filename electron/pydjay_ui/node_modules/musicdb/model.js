...
class MusicDatabase extends EventDispatcher {
    ...
    async updateTag(id, name, color) {
        ...
        if (tagObject) {
            ...
            await this.tags.setData(tagObject, {name: name, color: color})
            if (oldColor != color) {
                Object.values(await this.tracks.getAllObjects()).forEach(async (trackObject) => {
                    if (trackObject.metadata.tags && (trackObject.metadata.tags[0]==id)) {
                        await this.tracks.setData(trackObject, {"metadata.color": color})
                    }
                })
            }
        }
                ...
                if (trackObject.metadata.tags && (trackObject.metadata.tags.indexOf(id) != -1)) {
                    ...
                    if (tagIdx == 0) {
                        if (trackObject.metadata.tags.length > 0) {
                            let topTag = await this.tags.getObjectById(trackObject.metadata.tags[0])
